...
require File.join(File.dirname(__FILE__), 'country_phone_data')
module Validate
  class Phone
    NUMBER = '([^0][0-9]{1,7})$'  
    ...
    def self.split_to_parts(string, options = {})
      ...
      if country
        ...
        if options[:country_code]
          country = CountryPhoneData.find_by_country_code options[:country_code]
        end
      end
    ...
    def self.detect_country(string)
      ...
      CountryPhoneData.all.each_pair do |country_code, country|
